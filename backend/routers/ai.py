...
def search_hardware(
    ...
    **Full pipeline flow:**
    ...
    5. **Serialisation**: Each result row is converted to a column-keyed dict
       and validated through :class:`~backend.schemas.HardwareRead` before
       being returned.  This explicit validation step means a partial
       ``SELECT`` generated by the LLM (missing required columns such as
       ``id`` or ``status``) raises a clear HTTP 500 rather than a silent
       serialisation failure inside FastAPI's response pipeline.
    ...
    Returns:
        A list of :class:`~backend.schemas.HardwareRead` instances matching
    ...
    Raises:
        ...
        HTTPException (500): If SQL execution fails or if the LLM-generated
            query omits columns required by ``HardwareRead``.
    """
    ...
    # ── Step 5: Validate rows against HardwareRead ───────────────────────────
    # Explicit model_validate() catches partial SELECTs (missing required
    # columns like id/status) here with a meaningful error, rather than
    # letting FastAPI's serialiser crash silently after the function returns.
    columns: list[str] = list(result.keys())
    try:
        rows: list[HardwareRead] = [
            HardwareRead.model_validate(dict(zip(columns, row, strict=True)))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=(
                f"Search result could not be validated against the hardware schema. "
                f"The generated SQL may have omitted required columns: {exc}"
            ),
        ) from exc
